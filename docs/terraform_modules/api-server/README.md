...
| <a name="input_storage_driver"></a> [storage\_driver](#input\_storage\_driver) | Storage driver. The value should be one of: `s3`, `local`, `seaweedfs` | `string` | `"s3"` | no |
...
| <a name="input_storage_env_vars_seaweedfs"></a> [storage\_env\_vars\_seaweedfs](#input\_storage\_env\_vars\_seaweedfs) | The Lambda environment variables for the self-hosted SeaweedFS storage drivder. Required when `storage_driver` is `seaweedfs`. | <pre>object({<br>    STORAGE_SEAWEEDFS_BUCKET_NAME  = string<br>    STORAGE_SEAWEEDFS_USERNAME     = string<br>    STORAGE_SEAWEEDFS_PASSWORD     = string<br>    STORAGE_SEAWEEDFS_ENDPOINT_URL = string<br>  })</pre> | `null` | no |
