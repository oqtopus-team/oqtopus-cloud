...
Alembic is a lightweight database migration tool for usage with SQLAlchemy, the Python SQL toolkit and Object Relational Mapper (ORM). It helps manage schema changes over time, enabling version-controlled migrations in a consistent and repeatable way.
...
DB commands are defined in Makefile.

```makefile
up: ## Start the DB
	@docker compose up -d db
	@python scripts/mig-up.py
...
API commands are defined in Makefile.
...
OQTOPUS uses [Alembic](https://alembic.sqlalchemy.org/en/latest/) for database migration.

Migration commands are defined in Makefile.
...
mig-stamp: ## Stamp the revision
	@poetry run alembic stamp "$(REV)"
```

### Upgrade

  To upgrade DB, execute the command below:
...
### Downgrade
  ...
  To downgrade DB, execute the command below:

  ```makefile
  make mig-down REV="your_target_revision" # Replace the value with your revision.
  ```

### Generate migration file

  To generate a migration file, execute the command below:

  ```makefile
  make mig-gen ARG="your_message" # Replace the value with your comment.
  ```

### Stamp

  To stamp, execute the command below:

  ```makefile
  make mig-stamp REV="your_target_revision" # Replace the value with your revision.
  ```
