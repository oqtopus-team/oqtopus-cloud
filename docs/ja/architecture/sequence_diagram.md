...
> 例えば、ユーザーが QUEUED_FETCHED のタスク情報を Cloud から取得すると status は submitted になっており、Provider が取得すると status は QUEUED_FETCHED になっています。
...
> [!NOTE]
> (1) の時点でタスクが submitted 状態 (Provider によりフェッチされる前の状態) の場合、Cloud は即座にタスクを CANCELLED 状態に変更します。
